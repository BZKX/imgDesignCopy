...
import { MSG, type Language, type SelectionRect } from '@promptlens/core';
import { MODE_META, type Mode as InsightMode } from '@promptlens/core';
...
interface OverlayProps {
  ...
  /** Insight mode frozen at mount per plan §8a — single source of truth = chrome.storage.sync */
  mode: InsightMode;
  /** UI language frozen at mount. Defaults to 'zh'. */
  lang?: Language;
}

type SelectMode = 'rect' | 'element';

export function Overlay({ onComplete, onCancel, excludeHost, mode: insightMode, lang = 'zh' }: OverlayProps) {
  const modeMetaDisplay = lang === 'en' ? MODE_META[insightMode].displayName.en : MODE_META[insightMode].displayName['zh-CN'];
  const [mode, setMode] = useState<SelectMode>('element');
  ...
  return (
    ...
    >
      ...
      <div
        ...
        onClick={stopAll}
      >
        <span className="overlay-mode-badge" title={MODE_META[insightMode].description}>
          <span className="overlay-mode-icon"><ModeIcon mode={insightMode} /></span>
          {modeMetaDisplay}
        </span>
        <div className="overlay-segment" role="tablist">
          ...
          >
            <CursorIcon /> {t('overlay.mode.element', lang)}
          </button>
          <button
            type="button"
          ...
          >
            <RectIcon /> {t('overlay.mode.rect', lang)}
          </button>
        </div>
        <div className="overlay-hint">
          {mode === 'rect'
            ? t('overlay.hint.rect', lang)
            : t('overlay.hint.element', lang)}
          <span className="overlay-hint-sep" />
          <kbd>E</kbd> {t('overlay.hint.switch', lang)}
          <span className="overlay-hint-sep" />
          <kbd>Esc</kbd> {t('overlay.hint.cancel', lang)}
        </div>
      </div>
    </div>
  );
