  ...
  if (!dbPromise) {
    dbPromise = openDB(DB_NAME, DB_VERSION, {
      upgrade(db) {
        ...
        // v1 → v2: no-op schema change. Rows are normalized on read via
        // normalizeRow(); legacy `prompts` shape stays in place until touched.
      },
    });
  }
  return dbPromise;
...
function normalizeRow(raw: any): HistoryEntry {
  if (raw && raw.insight !== undefined && raw.mode) return raw as HistoryEntry;
  return {
    id: raw.id,
    createdAt: raw.createdAt,
    thumbnailBlob: raw.thumbnailBlob,
    pageUrl: raw.pageUrl,
...
export async function list(limit = 50): Promise<HistoryEntry[]> {
  ...
  while (cursor && out.length < limit) {
    out.push(normalizeRow(cursor.value));
